...
tests conf = testGroup "Private"
  [ testCase "getAccountList"
    (do as <- run_getAccountList conf
        case as of
          [] -> error "Received empty list of accounts"
          _  -> return ()
    )

  , testCase "getUSDAccount"
    (do as <- run_getAccountList conf
  ...
  , testCase "getOrder"
    (do no  <- creatNewLimitOrder
        oid <- run_placeOrder conf no
        threadDelay 1000000 -- 1 second delay
        o   <- run_getOrder conf (ocId oid)
  ...
  , testCase "getFills"
    (do oid <- run_placeOrder conf giveAwayOrder
        threadDelay 1000000 -- 1 second delay
        fs <- run_getFills conf (Just $ ocId oid) Nothing
        case fs of
...
-----------------------------------------------
-- DO NOT USE!! CAREFUL!! THIS WILL TRY TO SEND YOUR BITCOINS TO THE
-- SPECIFIED WALLET BELOW!  THIS MAY BE IRREVERSIBLE.
...
giveAwayOrder = NewMarketOrder
    ...
    -- LIVE ENVIRONMENT. YOU MAY LOSE MONEY.
    { noProductId = "BTC-USD"
    , noSide      = Sell
    , noSelfTrade = DecreaseAndCancel
    , noClientOid = Just $ ClientOrderId $ fromJust $ fromString "c2cc10e1-57d6-4b6f-9899-deadbeef2d8c"
     -- at most 1 BTC cent or 5 dollars per test
    , noSizeAndOrFunds = Right (Just 0.01, 5)
    }

creatNewLimitOrder :: IO NewOrder
creatNewLimitOrder = do
    -- can't be deterministic because exchange is stateful. Running a
    -- test twice with same random input may produce different results
    sz <- randomRIO (0,9999)
...
findUSDAccount as =
  case filter (\a -> accCurrency a == CurrencyId "USD") as of
    [a] -> a
    []  -> error "no USD denominated account found"
    _   -> error "more than one account denominated in USD found"

-----------------------------------------------
onSuccess :: (ToJSON a, FromJSON b)
          => ExchangeConf -> I.Request a b -> String -> IO b
onSuccess conf apicall errorstring = do
        r <- coinbaseRest conf apicall
        case r of
            Left  e -> print e >> error errorstring
            Right a -> return a

run_getAccountList :: ExchangeConf -> IO [Account]
run_getAccountList conf =
  onSuccess conf getAccountList "Failed to get account list"

run_getAccount :: ExchangeConf -> AccountId -> IO Account
run_getAccount conf acID =
  onSuccess conf (getAccount acID) "Failed to get account info"

run_getAccountLedger :: ExchangeConf -> AccountId -> IO [Entry]
run_getAccountLedger conf acID =
  onSuccess conf (getAccountLedger acID) "Failed to get account ledger"

run_placeOrder :: ExchangeConf -> NewOrder -> IO OrderConfirmation
run_placeOrder conf o =
  onSuccess conf (createOrder o) "Failed to create order"

run_getOrder :: ExchangeConf -> OrderId -> IO Order
run_getOrder conf oid =
  onSuccess conf (getOrder oid) "Failed to get order info"

run_getOrderList :: ExchangeConf -> [OrderStatus] -> IO [Order]
run_getOrderList conf ss =
  onSuccess conf (getOrderList ss) "Failed to get order list"

run_cancelOrder :: ExchangeConf -> OrderId -> IO [OrderId]
run_cancelOrder conf oid =
  onSuccess conf (cancelOrder oid) "Failed to cancel order"

run_getFills :: ExchangeConf -> Maybe OrderId -> Maybe ProductId -> IO [Fill]
run_getFills conf moid mpid =
  onSuccess conf (getFills moid mpid) "Failed to get fills"

run_withdrawBTC :: ExchangeConf -> CryptoWithdrawal -> IO CryptoWithdrawalResp
run_withdrawBTC conf w =
  onSuccess conf (createCryptoWithdrawal w) "Failed to create withdrawal"
